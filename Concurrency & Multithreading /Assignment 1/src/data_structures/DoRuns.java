...
public class DoRuns<T extends Comparable<T>> {
    ...
    //If {@literal >} 0, the threads spend some CPU time in between add orremove operations.
     ...
     * used.
     *
     * @param dataStructure - specifies which kind of data structure to use
     * @param nrThreads - specifies the number of threads to use
     * @param itemsToAdd - specifies the items to add
     * @param itemsToRemove - specifies the items to remove
     * @param workTime - if {@literal >} 0, the threads spend some CPU time in between add or remove operations.
     * @param debug - when set, the data structure is printed after adding the items
     */
    // 

    public DoRuns(String dataStructure, int nrThreads, T[] itemsToAdd, T[] itemsToRemove, int workTime, boolean debug){
    ...
    public void runDataStructure() {
        ...
        for (int run = 0; run < 1; run++) {
            ...
            for (int i = 0; i < nrThreads; i++) {
                ...
                T[] toRemove = Arrays.copyOfRange(itemsToRemove, i * sz,(i + 1) * sz);

                workerThreads.add(new WorkerThread<T>(i, sorted, toAdd, toRemove, workTime, barrier, debug));
            }
